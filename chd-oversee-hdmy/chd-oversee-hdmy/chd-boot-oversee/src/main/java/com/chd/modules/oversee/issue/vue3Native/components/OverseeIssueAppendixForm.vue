<template>
  <a-spin :spinning="confirmLoading">
    <a-form ref="formRef" class="antd-modal-form" :labelCol="labelCol" :wrapperCol="wrapperCol">
      <a-row>
        <a-col :span="24">
          <a-form-item label="附件路径地址" v-bind="validateInfos.appendixPath">
            <a-input v-model:value="formData.appendixPath" placeholder="请输入附件路径地址" :disabled="disabled"></a-input>
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <a-form-item label="附件类型 1 问题录入 2问题分配 3整改措施" v-bind="validateInfos.type">
	          <a-input-number v-model:value="formData.type" placeholder="请输入附件类型 1 问题录入 2问题分配 3整改措施" style="width: 100%" :disabled="disabled"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <a-form-item label="文件类型 0 无 1图片 2pdf 此字段不一定启用" v-bind="validateInfos.fileType">
	          <a-input-number v-model:value="formData.fileType" placeholder="请输入文件类型 0 无 1图片 2pdf 此字段不一定启用" style="width: 100%" :disabled="disabled"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
...
<script lang="ts" setup>
  ...
  import { saveOrUpdate } from '../OverseeIssueAppendix.api';
  ...
  const validatorRules = {
    type: [{ required: true, message: '请输入附件类型 1 问题录入 2问题分配 3整改措施!'},],
    fileType: [{ required: true, message: '请输入文件类型 0 无 1图片 2pdf 此字段不一定启用!'},],
