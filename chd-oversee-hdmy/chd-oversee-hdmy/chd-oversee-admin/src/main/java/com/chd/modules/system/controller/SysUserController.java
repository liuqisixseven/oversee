...
import com.chd.common.aspect.annotation.PermissionData;
...
import com.chd.modules.system.model.SysUserSysDepartModel;
import com.chd.modules.system.service.*;
import com.chd.modules.system.vo.SysDepartUsersVO;
import com.chd.modules.system.vo.SysUserRoleVO;
...
//      不是admin角色 只能查询到自己
        ...
        String departIdAndGetChildren = req.getParameter("departIdAndGetChildren");
        if(oConvertUtils.isNotEmpty(departIdAndGetChildren)){
            MyOrg myOrgById = myOrgService.getById(departIdAndGetChildren);
            if(null!=myOrgById&&StringUtil.isNotEmpty(myOrgById.getPath())){
                List<MyOrg> myOrgList = myOrgService.list(Wrappers.<MyOrg>lambdaQuery().likeRight(MyOrg::getPath, myOrgById.getPath()));
                if(null!=myOrgList&&myOrgList.size()>0){
                    List<String> orgIds = myOrgList.stream().map(MyOrg::getOrgId).collect(Collectors.toList());
                    if(oConvertUtils.listIsNotEmpty(orgIds)){
                        queryWrapper.in("hdmy_org_id",orgIds);
                    }
                }
            }
        }
        ...
        Set<String> roleSet = sysUserService.getUserRolesSet(loginUser.getUsername());
...
//        递归查询用户公司
        pageList.getRecords().stream().forEach((sysUserS)->handleSysUserOrganizeCompany(sysUserS));
    ...
    private String getOrganizeCompany(SysUser sysUser){
        if(null!=sysUser&&StringUtil.isNotEmpty(sysUser.getHdmyOrgId())){
            return myOrgService.getOrganizeCompanyOrgName(sysUser.getHdmyOrgId());
        }
        return null;
    }

    private void handleSysUserOrganizeCompany(SysUser sysUser){
        if(null!=sysUser&&StringUtil.isNotEmpty(sysUser.getHdmyOrgId())){
            MyOrg organizeCompany = myOrgService.getOrganizeCompany(sysUser.getHdmyOrgId());
            if(null!=organizeCompany){
                sysUser.setHdmyCompanyName(organizeCompany.getOrgName());
                sysUser.setHdmyCompanyOrgId(organizeCompany.getOrgId());
            }
        }
    }
    ...
    public Result<IPage<SysUser>> queryUserComponentData(
        ...
        if(StringUtils.isBlank(departId)){//增加权限限制，只能浏览用户的所属部门用户
            if(StringUtils.isNotBlank(user.getDepartIds())) {
                departId = user.getDepartIds().split(SymbolConstant.COMMA)[0];
            }else{
                return Result.OK(new Page<>(pageNo,pageSize));
            }
        }
        IPage<SysUser> pageList = sysUserDepartService.queryDepartUserPageList(departId, username, realname, pageSize, pageNo,id,user.getDepartIds());
