...
import com.chd.common.util.ImportExcelUtil;
...
public class SysDepartController {
    ...
    @ApiOperation(value = "查出我的部门树查询", notes = "查出我的部门树查询")
    ...
    @ApiOperation(value = "部门树查询", notes = "部门树查询")
    public Result<List<SysDepartTreeModel>> queryTreeList(@RequestParam(name = "ids", required = false) String ids, @RequestParam(name = "parentOrgIds", required = false) String parentOrgIds) {
        Result<List<SysDepartTreeModel>> result = new Result<>();
        try {
            // 从内存中读取
...
//			}
            if (oConvertUtils.isNotEmpty(parentOrgIds)) {
                List<SysDepartTreeModel> departList = sysDepartService.queryTreeListByParentOrgIds(parentOrgIds);
                result.setResult(departList);
            } else if (oConvertUtils.isNotEmpty(ids)) {
                ...
                if(StringUtil.isNotEmpty(user.getUsername())){
                    String redisKey = CommonConstant.USERNAME_ROLES_PREFIX + user.getUsername();
                    roles = (Set<String>) redisTemplate.opsForValue().get(redisKey);
                    if(null==roles||roles.size()<=0){
                        roles = commonAPI.queryUserRoles(user.getUsername());
                    }
                }
                if(CollectionUtils.isNotEmpty(roles) &&(roles.contains("admin")||roles.contains("superuser"))){
                    List<SysDepartTreeModel> list = sysDepartService.queryTreeList();
                    result.setResult(list);
                }else {
                    String departIds = user.getHdmyOrgId();
                    if (StringUtils.isNotBlank(departIds)) {
                        List<SysDepartTreeModel> list = sysDepartService.queryMyDeptTreeList(departIds);
                        result.setResult(list);
                    }
    ...
    public Result<List<SysDepartTreeModel>> queryDepartTreeSync(
        ...
        try {
            if (StringUtils.isBlank(parentId)) {
                ...
                String departIds = user.getHdmyOrgId();
                if (StringUtils.isNotBlank(departIds)) {
//					parentId=departIds.split(SymbolConstant.COMMA)[0];
                    ids = departIds.split(SymbolConstant.COMMA)[0];
                }
            }
            List<String> pathList = null;
            if (null != selectType) {
                pathList = new ArrayList<>();
                if (selectType.intValue() == 1) {
                    if (StringUtils.isNotBlank(ids)) {
                        String[] idArray = ids.split(SymbolConstant.COMMA);
                        if (null != idArray && idArray.length > 0) {
                            for (String myOrgId : idArray) {
                                MyOrg myOrgById = myOrgService.getById(myOrgId);
                                if (null != myOrgById && StringUtils.isNotBlank(myOrgById.getPath())) {
                                    pathList.add(myOrgById.getPath().replace(myOrgById.getOrgId(), ""));
                                }
                            }
                        }
                    }
            ...
            List<SysDepartTreeModel> list = sysDepartService.queryTreeListByPid(parentId, ids, primaryKey, pathList);
            result.setResult(list);
            result.setSuccess(true);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
