...
import com.chd.common.constant.FillRuleConstant;
import com.chd.common.constant.SymbolConstant;
import com.chd.common.util.FillRuleUtil;
import com.chd.common.util.YouBianCodeUtil;
import com.chd.common.util.oConvertUtils;
import com.chd.modules.system.entity.*;
import com.chd.modules.system.mapper.*;
import com.chd.modules.system.model.DepartIdModel;
import com.chd.modules.system.model.SysDepartTreeModel;
import com.chd.modules.system.service.ISysDepartService;
import com.chd.modules.system.util.FindsDepartsChildrenUtil;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	public List<SysDepartTreeModel> queryTreeList() {
		...
		query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
		query.orderByAsc(SysDepart::getDepartOrder);
		List<SysDepart> list = sysDepartSetFullName(this.list(query));
	...
	private List<SysDepart> sysDepartSetFullName(List<SysDepart> list){
		try{
			if(null!=list&&list.size()>0){
				for(SysDepart sysDepart : list){
					if(null!=sysDepart&&StringUtils.isNotEmpty(sysDepart.getId())){
						List<MyOrg> myOrgList = myOrgService.list(Wrappers.<MyOrg>lambdaQuery().eq(MyOrg::getOrgId, sysDepart.getId()));
						if(null!=myOrgList&&myOrgList.size()>0){
							MyOrg myOrg = myOrgList.get(0);
							if(null!=myOrg){
								if(StringUtils.isNotEmpty(myOrg.getOrgName())){
									sysDepart.setFullName(myOrg.getOrgName());
								}
								if(StringUtils.isNotEmpty(myOrg.getOrgShortName())){
									sysDepart.setShortName(myOrg.getOrgShortName());
								}
							}
						}
					}
	...
	@Override
	public List<SysDepartTreeModel> queryTreeListByParentOrgIds(String parentOrgIds) {
		List<SysDepartTreeModel> listResult=new ArrayList<>();

		if(oConvertUtils.isNotEmpty(parentOrgIds)){
			List<String> parentOrgIdList = Arrays.asList(parentOrgIds.split(","));
			if(CollectionUtils.isNotEmpty(parentOrgIdList)){
				for(String parentOrgId : parentOrgIdList){
					LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
					query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
					query.eq(true,SysDepart::getId,parentOrgId);
					query.orderByAsc(SysDepart::getDepartOrder);
					List<SysDepart> list= sysDepartSetFullName(this.list(query));
					for (SysDepart depart : list) {
						SysDepartTreeModel sysDepartTreeModel = new SysDepartTreeModel(depart);
						LambdaQueryWrapper<SysDepart> parentQuery = new LambdaQueryWrapper<SysDepart>();
						parentQuery.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
						parentQuery.eq(true,SysDepart::getParentId,depart.getId());
						parentQuery.orderByAsc(SysDepart::getDepartOrder);
						List<SysDepart> sysDepartListChildren = this.list(parentQuery);
						if(CollectionUtils.isNotEmpty(sysDepartListChildren)){
							String childrenIds = sysDepartListChildren.stream().filter((sysDepart) -> null != sysDepart && StringUtils.isNotEmpty(sysDepart.getId())).map((sysDepart) -> sysDepart.getId()).collect(Collectors.joining(","));
							if(StringUtils.isNotEmpty(childrenIds)){
								List<SysDepartTreeModel> sysDepartChildrenTreeModels = queryTreeListByParentOrgIds(childrenIds);
								if(null!=sysDepartChildrenTreeModels&&sysDepartChildrenTreeModels.size()>0){
									sysDepartTreeModel.setChildren(sysDepartChildrenTreeModels);
								}
							}
						}
	...
	public List<SysDepartTreeModel> queryTreeListByPid(String parentId,String ids, String primaryKey) {
		return queryTreeListByPid(parentId,ids,primaryKey,null);
	}
	 ...
	 */
	@Override
	public List<SysDepartTreeModel> queryTreeListByPid(String parentId, String ids, String primaryKey, List<String> pathList) {
		...
		if(null!=pathList&&pathList.size()>0){
			lqw.and(z -> {
				for(String pathLike : pathList){
						if(StringUtils.isNotEmpty(pathLike)){
							z.or().likeRight(SysDepart::getPath,pathLike);
						}
					}
				}
			);
