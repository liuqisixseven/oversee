...
import com.chd.modules.system.mapper.SysAnnouncementMapper;
...
public class SysAnnouncementServiceImpl extends ServiceImpl<SysAnnouncementMapper, SysAnnouncement> implements ISysAnnouncementService {
		...
		if(sysAnnouncement.getMsgType().equals(CommonConstant.MSG_TYPE_ALL)) {
			...
			if(sysAnnouncement.getMsgType().equals(CommonConstant.MSG_TYPE_UESR)){
				...
				for(int i=0;i<departIdArray.length;i++) {
					SysAnnouncementSend announcementSend = new SysAnnouncementSend();
					announcementSend.setAnntId(anntId);
					announcementSend.setDepartId(departIdArray[i]);
					announcementSend.setReadFlag(CommonConstant.NO_READ_FLAG);
					announcementSend.setReadTime(refDate);
					sysAnnouncementSendMapper.insert(announcementSend);
				}
	...
	@Override
	public List<SysAnnouncement> querySysCementPageByDepartId(String msgCategory, String departId) {
		if(StringUtil.isNotEmpty(departId)){
			return sysAnnouncementMapper.querySysCementListByDepartId(msgCategory,departId);
		}
		return null;
	}
	...
	public Page<SysAnnouncement> querySysCementPageByUserId(Page<SysAnnouncement> page, String userId, String msgCategory) {
		return querySysCementPageByUserId(page,userId,msgCategory,null);
	}

	@Override
	public Page<SysAnnouncement> querySysCementPageByUserId(Page<SysAnnouncement> page, String userId, String msgCategory,String departId) {
		if (page.getSize() == -1) {
			return page.setRecords(sysAnnouncementMapper.querySysCementListByUserId(null, userId, msgCategory,departId));
		} else {
			return page.setRecords(sysAnnouncementMapper.querySysCementListByUserId(page, userId, msgCategory,departId));
		}
	}
