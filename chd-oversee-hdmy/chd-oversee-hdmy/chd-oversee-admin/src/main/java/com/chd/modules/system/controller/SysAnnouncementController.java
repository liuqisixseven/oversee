...
import com.chd.common.constant.WebsocketConst;
...
import com.chd.common.util.TokenUtils;
...
import static com.chd.common.constant.CommonConstant.ANNOUNCEMENT_SEND_STATUS_1;
...
//		}
	...
	public Result<SysAnnouncement> doReleaseData(@RequestParam(name="id",required=true) String id, HttpServletRequest request) {
		...
		if(sysAnnouncement==null) {
			...
			if(ok) {
				...
				}else if(sysAnnouncement.getMsgType().equals(CommonConstant.MSG_TYPE_UESR)){
					// 2.插入用户通告阅读标记表记录
	...
	public Result<Map<String, Object>> listByUser(@RequestParam(required = false, defaultValue = "5") Integer pageSize) {
		...
		String hdmyOrgId = sysUser.getHdmyOrgId();
		...
		listByUserAddSysAnnouncementSendBySysAnnouncementList(announcements,userId,pageSize);

		List<SysAnnouncement> sysAnnouncements = sysAnnouncementService.querySysCementPageByDepartId("1", hdmyOrgId);
		listByUserAddSysAnnouncementSendBySysAnnouncementList(sysAnnouncements,userId,pageSize);
	...
	private void listByUserAddSysAnnouncementSendBySysAnnouncementList(List<SysAnnouncement> announcements,String userId,Integer pageSize){
