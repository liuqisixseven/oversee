        ...
        if(null!=myTaskVariables){
            ...
            if(null!=isSupervise&&isSupervise.intValue()==1){
                ...
                if(StringUtil.isNotEmpty(supervisorOrgIds)){
                    List<Map<String, Object>> subprocessUserDataList = getSubprocessUserDataList(supervisorOrgIds,"督办部门");
                            ...
                            if(null!=supervisorList){
                                for(Map<String, Object> supervisor : supervisorList){
                                    ...
                                    if(StringUtils.isNotEmpty(departId)&&departId.equals((String) subprocessUserData.get(WorkflowConstants.DEPART_ID_KEY))){
                                        subprocessUserData.put(WorkflowConstants.USER_ID_KEY,supervisor.get(WorkflowConstants.USER_ID_KEY));
                                    }
                                }
                            }
                            if(null!=form){
...
//                                                    System.out.println("------- responsibleUnitResponsibleDepartmentKey value : "+JSONObject.toJSONString(value));
    ...
    private void taskForm_selectDepart(Map<String,Object> form,String processId,WorkflowUserVo user,Map<String,Object> myTaskVariables){
        ...
        String responsibleCoordinationOrgIds=MapUtils.getString(form,"responsibleCoordinationOrgIds");
        String responsibleMainOrgIds=MapUtils.getString(form,"responsibleMainOrgIds");
        responsibleMainOrgIds = StringUtil.isNotEmpty(responsibleMainOrgIds)?responsibleMainOrgIds:departIds;
        //        责任部门设置
        if(StringUtil.isNotEmpty(responsibleMainOrgIds)||StringUtil.isNotEmpty(responsibleCoordinationOrgIds)){
            Assert.isTrue((StringUtil.isNotEmpty(responsibleMainOrgIds)), "请传递主要责任部门");
            if(StringUtil.isNotEmpty(responsibleMainOrgIds)){
                responsibleMainOrgIds = Arrays.asList((responsibleMainOrgIds).split(",")).stream().distinct().filter((orgId)->StringUtil.isNotEmpty(orgId)).collect(Collectors.joining(","));
            }
            if(StringUtil.isNotEmpty(responsibleCoordinationOrgIds)){
                responsibleCoordinationOrgIds = Arrays.asList((responsibleCoordinationOrgIds).split(",")).stream().distinct().filter((orgId)->StringUtil.isNotEmpty(orgId)).collect(Collectors.joining(","));
            }
            departIds = Arrays.asList((responsibleMainOrgIds + (StringUtil.isNotEmpty(responsibleCoordinationOrgIds)?("," + responsibleCoordinationOrgIds):"")).split(",")).stream().distinct().filter((orgId)->StringUtil.isNotEmpty(orgId)).collect(Collectors.joining(","));
        }
        if(StringUtils.isBlank(responsibleMainOrgIds)){
        ...
        List<Map<String, Object>> subprocessUserDataList = getSubprocessUserDataList(departIds,"责任部门");
        //如果是委派驳回时，需要将原始的责任业务人员带过去，否则后续再流程到整改无法找到责任业务员，流程无法流转
        // FIXME: by lhm 2022/12/27 如果修改责任部门时，对应的业务员还是之前责任部门的业务员的，会有问题
//        if(myTaskVariables.get(WorkflowConstants.RESPONSIBLE_UNIT_RESPONSIBLE_DEPARTMENT_LIST_KEY)!=null){
//            List<Map<String, Object>> oldUserDataList = (List<Map<String, Object>>) myTaskVariables.get(WorkflowConstants.RESPONSIBLE_UNIT_RESPONSIBLE_DEPARTMENT_LIST_KEY);
//            if(oldUserDataList.get(0).get(WorkflowConstants.USER_ID_KEY)!=null) {
//                subprocessUserDataList.get(0).put(WorkflowConstants.USER_ID_KEY,oldUserDataList.get(0).get(WorkflowConstants.USER_ID_KEY));
//            }
//        }
...
//        myTaskVariables.put(WorkflowConstants.responsibleUnitResponsibleDepartmentListKey + "Size",responsibleUnitResponsibleDepartmentList.size());
    ...
    @Override
    public List<Map<String, Object>> getSubprocessUserDataList(String orgIds,String orgDescribe) {
        List<Map<String, Object>> userDataList = new ArrayList<>();
        orgDescribe = StringUtils.isNotEmpty(orgDescribe) ? orgDescribe : "责任部门";
        if(StringUtil.isNotEmpty(orgIds)){
            ...
            for(String orgId : supervisorOrgIdArray){
                if(StringUtil.isNotEmpty(orgId)){
                    ...
                    AssertBizException.isTrue((null!=workflowManageUserVos&&workflowManageUserVos.size()>0),orgDescribe + "负责人不存在");
                    WorkflowUserVo workflowManageUserVo = workflowManageUserVos.get(0);
                    AssertBizException.isTrue((null!=workflowManageUserVo&&StringUtil.isNotEmpty(workflowManageUserVo.getId())),orgDescribe + "负责人不存在");
                    ...
                    AssertBizException.isTrue((null!=workflowSupervisorUserVos&&workflowSupervisorUserVos.size()>0),orgDescribe + "分管领导不存在");
                    WorkflowUserVo workflowSupervisorUserVo = workflowSupervisorUserVos.get(0);
                    AssertBizException.isTrue((null!=workflowSupervisorUserVo&&StringUtil.isNotEmpty(workflowSupervisorUserVo.getId())),orgDescribe + "分管领导不存在");
