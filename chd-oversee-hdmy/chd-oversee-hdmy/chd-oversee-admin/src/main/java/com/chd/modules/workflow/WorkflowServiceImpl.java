...
public class WorkflowServiceImpl implements WorkflowService {
    ...
    private List<WorkflowUserTaskVo> getNodeHandler(String processId) {
...
//            userTasks=workflowUserService.getBizVariableUser(userTasks,processVo.getCategory(),processVo.getBizId(),processVo.getStartUserId());
       ...
       return workflowVariablesService.setUserTaskListVariables(userTasks, taskVariables);
    }
    ...
    public Map<String, Object> updateProcessTaskInfoById(String id) {
        ...
        if (process != null) {
            try {
                List<WorkflowUserTaskVo> userTasks = findNextTaskByProcessId(id);//todo 多实例获取有问题？
                String taskKey = "";
                if (CollectionUtils.isNotEmpty(userTasks)) {
                    ...
                    if (CollectionUtils.isNotEmpty(nextUserTasks)) {
                        String nextUserTask = StringUtils.join(nextUserTasks.toArray(), ",");
                        String finalTaskKey = taskKey;
                        userTasks = getNodeHandler(id).stream().filter(a->a.getTaskKey().equals(finalTaskKey)).collect(Collectors.toList());
                        process.setNextUserTask(nextUserTask);
                        //从流程定义中获取到该环节处理人
                        if(userTasks.get(0)!=null && userTasks.get(0)!=null && CollectionUtils.isNotEmpty(userTasks.get(0).getVariableUser())){
                            WorkflowUserTaskVo workflowUserTaskVo =userTasks.get(0);
                            String allHanderPerson = workflowUserTaskVo.getVariableUser().get(0).getName();
                            if(allHanderPerson!=null && allHanderPerson.contains(nextUserTask)){
                                process.setNextUserTask(allHanderPerson);
                            }
                            process.setNextUserTask(moveOrgNamePerson(process.getNextUserTask()));
                        }
...
//                        workflowMessageService.sendNextTaskList(process.getBizId(),userTasks);
    ...
    private String moveOrgNamePerson(String allHanderPerson){
        String[] strArr = allHanderPerson.split(",");
...
//        设置督办人
        variables.put(WorkflowConstants.SUPERVISOR_LIST_KEY, workflowVariablesService.getSubprocessUserDataList(((String) variables.get("supervisorOrgIds")),"督办部门"));
//        设置责任部门
        variables.put(WorkflowConstants.RESPONSIBLE_UNIT_RESPONSIBLE_DEPARTMENT_LIST_KEY, workflowVariablesService.getSubprocessUserDataList((String) variables.get("responsibleOrgIds"),"责任部门"));
...
//                process.getBizId(), process.getVariables(), process.getTitle());
    ...
    public void launchCancelProcess(WorkflowProcess processInstance) {
        ...
        if(variables.get("processId")==null){
...
//                    }
    ...
    @Override
    public WorkflowTaskVo findTaskById(String taskId,LoginUser sysUser) {
        ...
        if (task != null) {
            ...
            Long issueId = null;
            if (process != null) {
                String bizId = process.getBizId();
                if(StringUtils.isNotEmpty(bizId)){
                    workflowTaskVo.setBizId(bizId);
                    try{
                        issueId = Long.parseLong(bizId);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
                }
            ...
            OverseeIssue redisCacheOverseeIssue = null;
            if(null!=issueId&&issueId.longValue()>0L){
                redisCacheOverseeIssue = overseeIssueService.getRedisCacheOverseeIssue(issueId);
                if(null!=redisCacheOverseeIssue){
                    workflowTaskVo.getMyTaskVariables().put(WorkflowConstants.RESPONSIBLE_ORG_IDS_KEY, getIssueDepartIds(workflowTaskVo.getProcessId(),redisCacheOverseeIssue.getResponsibleOrgIds()));
                }
            }

            if (StringUtils.isNotBlank(task.getTaskDefinitionKey())) {
                String processStage = (String) taskService.getVariableLocal(task.getId(), BaseConstant.PROCESS_STAGE_KEY);
                if (null == processStage) {
                    processStage = "";
                    List<ExtensionAttribute> extensionAttribute = WorkflowFlowElementUtils.getExtensionAttribute(task.getTaskDefinitionKey(), task.getProcessDefinitionId(), BaseConstant.PROCESS_STAGE_KEY);
                    if (null != extensionAttribute && extensionAttribute.size() > 0) {
                        processStage = extensionAttribute.get(0).getValue();
                    }
                    taskService.setVariableLocal(task.getId(), BaseConstant.PROCESS_STAGE_KEY, processStage);
                }
                workflowTaskVo.getMyTaskVariables().put("processStage", processStage);
            }
            ...
            if(StringUtils.isNotEmpty(task.getFormKey())){
                formKeyList = Arrays.asList(task.getFormKey().split(";"));
            }


//            督办环节查询绑定责任部门ids
            workflowTaskVo.getMyTaskVariables().put(WorkflowConstants.SUPERVISOR_DATA_KEY, getSupervisorDataByTask(task,issueId));

            if (formKeyList.contains(WorkflowConstants.TASK_FORMID.DISTRIBUTION_SUPERVISION_DEPARTMENT) || WorkflowConstants.TASK_FORMID.DISTRIBUTION_SUPERVISION_DEPARTMENT.equals(task.getFormKey())) {
                Map<String, Object> myTaskVariables = workflowTaskVo.getMyTaskVariables();
                if(null!=issueId&&issueId.longValue()>0L){
                    if(null!=redisCacheOverseeIssue){
                        if(StringUtils.isNotEmpty(redisCacheOverseeIssue.getResponsibleOrgIds())){
                            myTaskVariables.put(WorkflowConstants.RESPONSIBLE_ORG_IDS_KEY, redisCacheOverseeIssue.getResponsibleOrgIds());
                        }
                        myTaskVariables.put(WorkflowConstants.IS_SUPERVISE_KEY, redisCacheOverseeIssue.getIsSupervise());
                        myTaskVariables.put(WorkflowConstants.SUPERVISOR_ORG_IDS_KEY, redisCacheOverseeIssue.getSupervisorOrgIds());
                        myTaskVariables.put(WorkflowConstants.SEVERITY_KEY, redisCacheOverseeIssue.getSeverity());
                    }

                    Map<String, Object> selectMap = Maps.newHashMap();
                    selectMap.put("issueId",issueId);
                    selectMap.put("issuesProcessType", IssuesSupervisor.issuesProcessMainType);
                    myTaskVariables.put(WorkflowConstants.ISSUES_SUPERVISOR_LIST_KEY, issuesSupervisorService.selectIssuesSupervisorList(selectMap));
                }
            }

            if (formKeyList.contains(WorkflowConstants.TASK_FORMID.DEPTS_SELECT) || WorkflowConstants.TASK_FORMID.DEPTS_SELECT.equals(task.getFormKey())) {
                Map<String, Object> myTaskVariables = workflowTaskVo.getMyTaskVariables();
                ...
                if(null!=issueId&&issueId.longValue()>0L){
                    if(null!=redisCacheOverseeIssue){
                        if(StringUtils.isEmpty(redisCacheOverseeIssue.getResponsibleMainOrgIds())){
                            redisCacheOverseeIssue.setResponsibleMainOrgIds(redisCacheOverseeIssue.getResponsibleOrgIds());
                        }
                        myTaskVariables.put(WorkflowConstants.RESPONSIBLE_ORG_IDS_KEY, redisCacheOverseeIssue.getResponsibleOrgIds());
                        myTaskVariables.put(WorkflowConstants.RESPONSIBLE_MAIN_ORG_IDS_KEY, redisCacheOverseeIssue.getResponsibleMainOrgIds());
                        myTaskVariables.put(WorkflowConstants.RESPONSIBLE_COORDINATION_ORG_IDS_KEY, redisCacheOverseeIssue.getResponsibleCoordinationOrgIds());
                    }
                }
            }

//            taskService.getVariableLocal(task.getId(), WorkflowConstants.FORM_MAP_KEY);

            if (formKeyList.contains(WorkflowConstants.TASK_FORMID.PIN_NUMBER) || formKeyList.contains(WorkflowConstants.TASK_FORMID.TEXT_AND_TIME) || WorkflowConstants.TASK_FORMID.PIN_NUMBER.equals(task.getFormKey()) || WorkflowConstants.TASK_FORMID.TEXT_AND_TIME.equals(task.getFormKey())) {
                ...
                if (null != responsibleUnitResponsibleDepartment) {
                    departId = (String) responsibleUnitResponsibleDepartment.get(WorkflowConstants.DEPART_ID_KEY);
                    myTaskVariables.put(WorkflowConstants.DEPART_ID_KEY, departId);
                }

                if(StringUtils.isEmpty(departId)&&null!=sysUser&&StringUtils.isNotEmpty(sysUser.getHdmyUserId())){
                    MyUser myUserById = myUserService.getById(sysUser.getHdmyUserId());
                    if(null!=myUserById&&StringUtils.isNotEmpty(myUserById.getOrgId())){
                        departId = myUserById.getOrgId();
                    }
                }

                if (formKeyList.contains(WorkflowConstants.TASK_FORMID.TEXT_AND_TIME) || WorkflowConstants.TASK_FORMID.TEXT_AND_TIME.equals(task.getFormKey())){
                    myTaskVariables.put(WorkflowConstants.FORM_MAP_KEY, getTextAndTimeDataForm(issueId,departId));
                }

                if (formKeyList.contains(WorkflowConstants.TASK_FORMID.PIN_NUMBER) || WorkflowConstants.TASK_FORMID.PIN_NUMBER.equals(task.getFormKey())){
                    myTaskVariables.put(WorkflowConstants.FORM_MAP_KEY, getPinNumberDataForm(issueId,departId));
                }
...
//                         if(WorkflowConstants.TASK_FORMID.PIN_NUMBER.equals(task.getFormKey())){
            Map<String, Object> myTaskVariables = workflowTaskVo.getMyTaskVariables();
            Map<String, Object> form = (Map<String, Object>) taskService.getVariableLocal(task.getId(), WorkflowConstants.FORM_MAP_KEY);
            if(null!=form){
                myTaskVariables.put(WorkflowConstants.FORM_MAP_KEY, form);
            }
...
//             }
    ...
    public String getIssueDepartIds(String processId, String ids) {
        if(StringUtils.isNotEmpty(ids)){
            List<WorkflowUserTaskVo> workflowUserTaskVos = this.findNextTaskByProcessId(processId);
            ...
            if (oConvertUtils.isNotEmpty(ids)) {
                ...
                if(CollectionUtils.isNotEmpty(departList)){
                    String sysDepartIds = departList.stream().filter((sysDepartTreeModel) -> null != sysDepartTreeModel && StringUtils.isNotEmpty(sysDepartTreeModel.getId())).map((sysDepartTreeModel) -> sysDepartTreeModel.getId()).collect(Collectors.joining(","));
                    if(StringUtils.isNotEmpty(sysDepartIds)){
                        return sysDepartIds;
                    }
                }
            }
    ...
    public Map<String,Object> getSupervisorDataByTask(Task task,Long issueId){
        Map<String, Object> supervisorData = null;
        if(null!=task&&StringUtils.isNotEmpty(task.getExecutionId())){
            supervisorData = (Map<String, Object>) runtimeService.getVariableLocal(runtimeService.createExecutionQuery().executionId(task.getExecutionId()).singleResult().getParentId(), WorkflowConstants.SUPERVISOR_KEY);
            if(null!=supervisorData&&supervisorData.size()>0){
                String departId = (String) supervisorData.get(WorkflowConstants.DEPART_ID_KEY);
                if(StringUtils.isNotEmpty(departId)&&null!=issueId&&issueId.longValue()>0L){
                    Map<String, Object> selectMap = Maps.newHashMap();
                    selectMap.put("issueId",issueId);
                    selectMap.put("issuesProcessType", IssuesSupervisor.issuesProcessMainType);
                    selectMap.put("supervisorOrgId", departId);
                    List<IssuesSupervisor> issuesSupervisors = issuesSupervisorService.selectIssuesSupervisorList(selectMap);
                    if(null!=issuesSupervisors&&issuesSupervisors.size()>0){
                        supervisorData.put(WorkflowConstants.BIND_RESPONSIBLE_ORG_IDS_KEY, issuesSupervisors.stream().filter((issuesSupervisor)->null!=issuesSupervisor&&StringUtils.isNotEmpty(issuesSupervisor.getBindResponsibleOrgIds())).map((issuesSupervisor)->issuesSupervisor.getBindResponsibleOrgIds()).collect(Collectors.joining(",")));
                    }
                }
            }
        }
    ...
    public void getPreviousTaskDataByCurrentTask(Task task, WorkflowTaskVo workflowTaskVo) {
        if (task != null && null != workflowTaskVo) {
            ...
            if (StringUtils.isNotEmpty(previousTaskDataSrc)) {
                ...
                if(StringUtils.isEmpty(workflowTaskVo.getPreviousTaskUserNames())) {
                    List<WorkflowComment> comments = workflowCommentService.getCommentByProcessInstanceId(task.getProcessInstanceId());
                    workflowTaskVo.setPreviousTaskUserNames(comments.get(0).getUserName());
                }
            } else {
...
//        List<UserTask> userTasks= workflowModelService.getDepolymentUserTask(processInstance.getDeploymentId());
    ...
    private Map<String, Object> getTextAndTimeDataForm(Long issueId,String departId){
        if(null!=issueId&&issueId.longValue()>0L){
            if(StringUtils.isNotEmpty(departId)){
                List<IssuesRectificationMeasure> issuesRectificationMeasureList = issuesRectificationMeasureService.list(Wrappers.<IssuesRectificationMeasure>lambdaQuery().eq(IssuesRectificationMeasure::getIssueId, issueId).eq(IssuesRectificationMeasure::getOrgId, departId).eq(IssuesRectificationMeasure::getDataType, 1));
                if(CollectionUtils.isNotEmpty(issuesRectificationMeasureList)){
                    Map<String, Object> form = Maps.newHashMap();
                    form.put("issuesRectificationMeasureList",issuesRectificationMeasureList);
                    return form;
                }
            }
        }
    ...
    private Map<String, Object> getPinNumberDataForm(Long issueId,String departId){
        if(null!=issueId&&issueId.longValue()>0L){
            if(StringUtils.isNotEmpty(departId)){
                Map<String, Object> form = Maps.newHashMap();
...
//                if(null!=redisCacheOverseeIssue){
//                    form.put("submitState",redisCacheOverseeIssue.getSubmitState());
//                }



                Map<String, Object> selectMap = Maps.newHashMap();
                selectMap.put("overseeIssueId", issueId);
                selectMap.put("orgIdList", Arrays.asList(new String[]{departId}));
                Map<String, Object> cancelANumberMap = reasonCancellationService.getCancelANumber(selectMap);
                if(null!=cancelANumberMap){
                    form.put("accountabilityHandlingList",cancelANumberMap.get("accountabilityHandlingList"));
                    form.put("rectifyViolationsList",cancelANumberMap.get("rectifyViolationsList"));
                    form.put("recoverFundsList",cancelANumberMap.get("recoverFundsList"));
                    form.put("improveRegulationsList",cancelANumberMap.get("improveRegulationsList"));


                    List<ReasonCancellation> reasonCancellationList = (List<ReasonCancellation>) cancelANumberMap.get("reasonCancellationList");
                    if(CollectionUtils.isNotEmpty(reasonCancellationList)){
                        ReasonCancellation reasonCancellation = reasonCancellationList.get(0);
                        if(null!=reasonCancellation&&StringUtils.isNotEmpty(reasonCancellation.getReason())){
                            form.put("text",reasonCancellation.getReason());
                        }
                    }
                }
