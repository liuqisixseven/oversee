...
import com.chd.modules.system.model.SysPermissionTree;
import com.chd.modules.system.model.TreeModel;
import com.chd.modules.system.service.*;
import com.chd.modules.system.util.PermissionDataUtil;
...
//	}
	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request) {
		...
		try {
			...
			if(null!=menujsonArray&&menujsonArray.size()>0){
				for(int i=0;i<menujsonArray.size();i++){
					JSONObject menujson = (JSONObject) menujsonArray.get(i);
					if(null!=menujson){

						JSONObject meta = menujson.getJSONObject("meta");
						if(null==meta){
							meta = new JSONObject();
						...
						//			TODO 暂时直接查询
						if(StringUtil.isNotEmpty(title)&&title.equals("我的待办")){
							try{
								Long count = workflowTaskService.countUserTodoTotal(loginUser.getId());
								String countSrc = (null!=count&&count.longValue()>0L)?count+"":"";
								meta.put("messagesCount",countSrc);
							}catch (Exception e){
								e.printStackTrace();
							}
						}
			...
			json.put("sysSafeMode", chdBaseConfig.getSafeMode());
	...
	public Result<?> getPermCode() {
		try {
            ...
            // 系统安全模式
			result.put("sysSafeMode", chdBaseConfig.getSafeMode());
	...
	public Result<SysPermission> add(@RequestBody SysPermission permission) {
		...
		try {
			...
			if(permission.getRequiredPermissions()==null){
				permission.setRequiredPermissions(true);
			}
