...
import com.chd.modules.system.model.SysLoginModel;
import com.chd.modules.system.service.*;
import com.chd.modules.system.service.impl.SysBaseApiImpl;
import com.chd.modules.system.util.RandImageUtil;
...
public class LoginController {
	...
	private static String[] defaultRoleCodeArray = new String[]{"USER_WORKFLOW"};

	private final String BASE_CHECK_CODES = "wertyuiplkjhfdsazxcvbnmQWERTYUPLKJHGFDSAZXCVBNM2345678";
...
//            log.warn("验证码错误，key= {} , Ui checkCode= {}, Redis checkCode = {}", sysLoginModel.getCheckKey(), lowerCaseCaptcha, checkCode);
	...
	@ApiOperation("登录接口")
	@RequestMapping(value = "/authorizedLogin", method = RequestMethod.POST)
	public Result<JSONObject> authorizedLogin(@RequestBody SysLoginModel sysLoginModel){
		try {
			Result<JSONObject> result = new Result<JSONObject>();
			String hdmyUserId = sysLoginModel.getHdmyUserId();
			String sessionId = sysLoginModel.getSessionId();
			String t = StringUtil.isNotEmpty(sysLoginModel.getT())?sysLoginModel.getT() : new Date().getTime()+"";

			Assert.isTrue((StringUtil.isNotEmpty(hdmyUserId)),"未获取到userId");


			//1. 校验用户是否有效
			LambdaQueryWrapper<SysUser> queryWrapper = new LambdaQueryWrapper<>();
			queryWrapper.eq(SysUser::getHdmyUserId,hdmyUserId);
			SysUser sysUser = sysUserService.getOne(queryWrapper);

			if(StringUtil.isNotEmpty(myauthUrl)){
				try{
					Assert.isTrue((StringUtil.isNotEmpty(sessionId)),"未获取到sessionid");
					String urlxt = (myauthUrl + sessionId+"/"+ JsToolUtil.encodeURIComponent(hdmyUserId) +"/"+t);
					System.out.println("urlxt : " + urlxt);
					String responseContent = HttpClientUtil.HttpGet(urlxt);
					Assert.isTrue((StringUtil.isNotEmpty(responseContent)),"未获取到协同用户验证返回数据");
					System.out.println("responseContent : " + responseContent);
					JSONObject jsonObject = JSONObject.parseObject(responseContent);
					Assert.isTrue((null!=jsonObject),"协同用户验证返回数据异常");
					Integer code = jsonObject.getInteger("code");
					Assert.isTrue((null!=code&&code.intValue()==200),"协同用户验证未通过，请重新登录协同系统");
			...
			LoginUser loginUser = new LoginUser();
			BeanUtils.copyProperties(sysUser, loginUser);
			baseCommonService.addLog("authorizedLogin 用户名: " + sysUser.getUsername() + ",登录成功！", CommonConstant.LOG_TYPE_1, null,loginUser);
			return result;
...
//		获取登录公告
		List<SysAnnouncement> sysAnnouncementList = sysAnnouncementService.list(Wrappers.<SysAnnouncement>lambdaQuery().eq(SysAnnouncement::getMsgCategory, "3").eq(SysAnnouncement::getSendStatus, "1").eq(SysAnnouncement::getDelFlag, 0).orderByDesc(SysAnnouncement::getCancelTime));
		if(null!=sysAnnouncementList&&sysAnnouncementList.size()>0){
			obj.put("loginSysAnnouncement", sysAnnouncementList.get(0));
		}

		obj.put("systemNameDictList", sysDictService.getDictItems(BaseConstant.SYSTEM_NAME_DICT_KEY));
		...
		result.setResult(obj);
		result.success("登录成功");
		checkUserDefaultSysRole(sysUser);
		return result;
	}

	private void checkUserDefaultSysRole(SysUser sysUser){
		if(null!= defaultRoleCodeArray && defaultRoleCodeArray.length>0){
			if(null!=sysUser&&StringUtil.isNotEmpty(sysUser.getId())){
				for(String defaultRoleCode : defaultRoleCodeArray){
					if(StringUtil.isNotEmpty(defaultRoleCode)){
						List<SysRole> sysRoleList = sysRoleService.list(Wrappers.<SysRole>lambdaQuery().eq(SysRole::getRoleCode, defaultRoleCode));
						if(null!=sysRoleList&&sysRoleList.size()>0){
							for(SysRole sysRole : sysRoleList){
								if(null!=sysRole&&StringUtil.isNotEmpty(sysRole.getId())&&defaultRoleCode.equals(sysRole.getRoleCode())){
									List<SysUserRole> sysUserRoleList = sysUserRoleService.list(Wrappers.<SysUserRole>lambdaQuery().eq(SysUserRole::getUserId, sysUser.getId()).eq(SysUserRole::getRoleId, sysRole.getId()));
									if(null==sysUserRoleList||sysUserRoleList.size()<=0){
										synchronized (SysUserRole.class){
											sysUserRoleList = sysUserRoleService.list(Wrappers.<SysUserRole>lambdaQuery().eq(SysUserRole::getUserId, sysUser.getId()).eq(SysUserRole::getRoleId, sysRole.getId()));
											if(null==sysUserRoleList||sysUserRoleList.size()<=0){
