...
public abstract class MultiInstanceActivityBehavior extends FlowNodeActivityBehavior implements SubProcessActivityBehavior {
    ...
    protected void cleanupMiRoot(DelegateExecution execution) {
        ...
        if(multiInstanceRootExecution!=null) {
            ...
            parentExecution = multiInstanceRootExecution.getParent();
            ...
            parentExecution=(ExecutionEntity)execution.getParent();
    ...
    protected FlowableMultiInstanceActivityCompletedEvent buildCompletedEvent(DelegateExecution execution, FlowableEngineEventType eventType) {
        ...
        return FlowableEventBuilder.createMultiInstanceActivityCompletedEvent(eventType,
                execution.getVariable(NUMBER_OF_INSTANCES)==null?0:(int)execution.getVariable(NUMBER_OF_INSTANCES),
                execution.getVariable(NUMBER_OF_ACTIVE_INSTANCES)==null?0:(int) execution.getVariable(NUMBER_OF_ACTIVE_INSTANCES),
                execution.getVariable(NUMBER_OF_COMPLETED_INSTANCES)==null?0:(int) execution.getVariable(NUMBER_OF_COMPLETED_INSTANCES),
                flowNode.getId(),
